...
public class OlapUtils {
	...
	public JSONObject getOlapCubes() throws JSONException {
		...
		for (MondrianCatalog catalog : catalogList) {
			...
			Connection connection = getMdxConnection(catalog.getDefinition(), catalog.getEffectiveDataSource().getJndi());
			if (connection == null) {
	    ...
	    for (Cube mdCube : mdCubes) {
	    	if(mdCube.name.equals(cube)) {
		    	...
		    	for (CalculatedMember mdCalculatedMember : mdCalculatedMembers) {
		    		String formatString = mdCalculatedMember.formatString;
		    		for (CalculatedMemberProperty prop : mdCalculatedMember.memberProperties) {
		    		...
		    		jsonMdCalculatedMember.put("formatString", formatString);
	...
	private Connection getMdxConnection(String catalog) {
		...
		return getMdxConnection(selectedCatalog.getDefinition(), selectedCatalog.getEffectiveDataSource().getJndi());		
	}
