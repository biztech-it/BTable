...
bt.components.BTable = function(spec) {
	...
	myself.getMenuTitle = function(btRef) {
		...
		if(member != level && level.indexOf("].[") > -1 && (level.indexOf("[Measures]") < 0 || "[Measures].[" + member + "]" != level))
			title += "<br />" + (member == "BT_TOTAL" ? "Total" : (member == null ? "<i>NULL</i>" : (!isNaN(parseFloat(member)) && member.toString().indexOf(" ") < 0 && !btRef.colspan ? sprintf(/*"%.2f"*/"%d", member) : member)));
	...
	myself.printFilters = function() {
		...
		$.each(dimensions, function(i, v) {
			...
			if(filterMode == "exclude")
				prettyFilter = "<i>all except</i> " + prettyFilter;
			else if(filterMode == "between")
				prettyFilter = "<i>between</i> " + prettyFilter.replace(" , ", " <i>and</i> ");

			if(hierarchy != previousHierarchy)
				html += "&nbsp;&nbsp;<span class='hierarchy'>" + hierarchy + "</span>";
			html += "<span class='separator'> >> </span><a href='#' class='level' data-qn='" + qualifiedName + "'>" + name + "</a>: <span class='filter'>" + prettyFilter + "</span>";
		...
		html = "<a href='#' class='filtersTitle' data-qn=''>FILTERS</a>:&nbsp;&nbsp;<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/" + prefix + "locked.png' class='" + prefix + "lockedIcon' />" + html;
		...
		if(dimensionsAxisSorted || measuresAxisSorted) {
			...
			if(columnsAxisName != "") {
				...
				if(dir != "ASC")
					html += "<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/asc.png' class='sortIcon' title='ASC' data-target='" + axisLetter + "' data-by='" + by + "' data-dir='ASC' />";
				if(dir != "BASC")
					html += "<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/basc.png' class='sortIcon' title='BASC' data-target='" + axisLetter + "' data-by='" + by + "' data-dir='BASC' />";
				if(dir != "DESC")
					html += "<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/desc.png' class='sortIcon' title='DESC' data-target='" + axisLetter + "' data-by='" + by + "' data-dir='DESC' />";
				if(dir != "BDESC")
					html += "<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/bdesc.png' class='sortIcon' title='BDESC' data-target='" + axisLetter + "' data-by='" + by + "' data-dir='BDESC' />";
				html += "<img src='" + Dashboards.getWebAppPath() + "/content/BTable/static/custom/img/icons/clear.png' class='sortIcon' title='Clear' data-target='" + axisLetter + "' data-by='' data-dir='' />";
			}
			if(html != "")
				html += "&nbsp;&nbsp;";
			if(rowsAxisName != "") {
			...
			html = "<div style='margin-top:5px'><span class='filtersTitle'>SORTS</span>: " + html + "</div>";
	...
	myself.openFiltersSelectorPanel = function(selectedLevel) {
		...
		html += "<span class='btTitle'>Filters</span><button id='dashboardBindingButton'>" + (filtersMap.synchronizedByParameters ? "Unbind from" : "Bind to") + " dashboard</button>";

		html += "</div><div id='filterSelectorTitle' class='topBoxRight'>Select from the list on the left, the level to be filtered.";
		...
		$.each(hierarchyQualifiedNames, function(i, key) {
			...
			$.each(hierarchy.order, function(i, v) {
				...
				var isAllMember = selections > 0 && (firstMember == "All" || firstMember.indexOf("All ") == 0 || firstMember.indexOf(".[All]") > 0 || firstMember.indexOf(".[All ") > 0); 

				var filterString = "";
				if(filtered && !isAllMember)
					filterString = (synchronizedByParameters && filtersMap.synchronizedByParameters)? "dashboard parameter" : (filterMode == "between" ? filterMode + " " + selections : selections + " to " + filterMode);
		...
		var multiselectBetweenModeProperties = {
			header: "Choose a maximum of 2 items!",
			minWidth: 480,
			height: 370,
			noneSelectedText: 'Select options',
			selectedList: 2,
		...
		var generateFilterSelector = function(levelQualifiedName, levelLabel) {	
			...
			html += "<div><strong style='font-weight:bold'>Filter mode:</strong> <input type='radio' name='filter-mode' value='in' " + (level.filterMode == "between" ? "" : "checked") + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + "/>In";
			html += "<input type='radio' name='filter-mode' value='between' " + (level.filterMode == "between" ? "checked" : "") + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + "/>Between";
			html += "</div><div id='exceptBox'" + (level.filterMode == "between" ? " style='display:none'" : "") + "><strong>Except:</strong> <input type='checkbox' name='except' value='except' " + (level.filterMode == "exclude" ? "checked" : "") + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + "/>";
			html += "</div><div><strong style='font-weight:bold'>Unique names:</strong> <input type='radio' name='uniquenames' value='yes' " + (uniqueNames ? "checked" : "") + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + "/>yes";
			html += "<input type='radio' name='uniquenames' value='no' " + (uniqueNames ? "" : "checked") + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + "/>no";
			...
			html += "<div class='updateFilterBar'><input id='update-filter' type='button' value='Update Filter'" + (boundToDashboard && level.synchronizedByParameters ? " disabled" : "") + " /></div>";
			
			$("#filterSelectorStage").html(html);
			
			var membersSelectObj = $("#membersSelect");
			membersSelectObj.multiselect(level.filterMode == "between" ? multiselectBetweenModeProperties : multiselectDefaultProperties).multiselectfilter();
			...
			$("input:radio[name='filter-mode']").change(function() {
				if($("input:radio[name='filter-mode']:checked").val() == 'between') {
					$("#exceptBox").hide();
					membersSelectObj.multiselect("uncheckAll").multiselectfilter("destroy").multiselect("destroy").multiselect(multiselectBetweenModeProperties).multiselectfilter();
				}
				else {
					$("#exceptBox").show();
					membersSelectObj.multiselect("uncheckAll").multiselectfilter("destroy").multiselect("destroy").multiselect(multiselectDefaultProperties).multiselectfilter();
				}
			});
			...
			$("input:radio[name='uniquenames']").change(function() {
				...
				membersSelectObj.multiselect(isBetweenMode ? multiselectBetweenModeProperties : multiselectDefaultProperties).multiselectfilter();
				if(uniqueNames) {
					membersSelectObj.multiselect({
					   selectedText: function(numChecked, numTotal, checkedItems){
						  var items = $.map(checkedItems, function(e) {
			...
			$("#update-filter").click(function() {
				...
				if(members == null) {
					...
					level.filtered = true;
					filterString = filterMode == "between" ? filterMode + " " + members.length : members.length + " to " + filterMode;
				}
		...
		$("#dashboardBindingButton").bind("click", function() {
				...
				if($(this).data("par")) {
					...
					if(bound) {
						...
						if(isSelected) {
							...
							membersSelectObj.multiselect(filterMode == "between" ? multiselectBetweenModeProperties : multiselectDefaultProperties).multiselectfilter();
						...
						filterInfoObj.text("dashboard parameter");
					}
					else {
						var filterMode = level.filterMode;
						...
						var firstMember = selectedMembers[0];
						var isAllMember = firstMember == "All" || firstMember.indexOf("All ") == 0 || firstMember.indexOf(".[All]") > 0 || firstMember.indexOf(".[All ") > 0; 
						if(isAllMember)
							filterInfoObj.text("");
						else
							filterInfoObj.text(filterMode == "between" ? filterMode + " " + selectedMembers.length : selectedMembers.length + " to " + filterMode);
					}
			...
			$(this).text(bound ? "Unbind from dashboard" : "Bind to dashboard");
	...
	myself.getExtremeLevelsInHierarchyInAxis = function(targetQualifiedName, axisQualifiedNames) {
 	...
 	myself.buildHeaderContextMenu = function(target) {
		...
		if(targetType == "DIMENSION" || targetType == "PIVOT_DIMENSION") {							
			...
			menu.add = {name: "Add", items: {}, disabled: newLevelsAsObjects.length == 0};
			menu.change = {name: "Change", items: {}, disabled: newLevelsAsObjects.length == 0};
			menu.remove = {
				name: "Remove",
			...
			$.each(hierarchies, function(i, hierarchy) {
				...
				if(levels.length == 1 && levels[0].name == hierarchy && levels[0].depth == 1) {
					...
					if($.inArray(hierarchy, axisHierarchies) > -1) {
						...
						menu.add.items["add-dim-" + i].items["add-dim-" + i + "-before"].name = "Before";
						...
						menu.add.items["add-dim-" + i].items["add-dim-" + i + "-after"].name = "After";
					...
					$.each(levels, function(j, level) {
						...
						if($.inArray(hierarchy, axisHierarchies) > -1) {
							...
							menu.add.items["add-dim-" + i].items["add-dim-" + i + "-" + j].items["add-dim-" + i + "-" + j + "-before"].name = "Before";
							...
							menu.add.items["add-dim-" + i].items["add-dim-" + i + "-" + j].items["add-dim-" + i + "-" + j + "-after"].name = "After";
			...
			if(targetType == "MEASURE") {				
				...
				menu.add = {name: "Add", items: {}, disabled: newMeasures.length == 0};
				menu.change = {name: "Change", items: {}, disabled: newMeasures.length == 0};
				menu.remove = {
					name: "Remove",
				...
				$.each(newMeasures, function(i, v) {
					...
					menu.add.items["add-kpi-" + i].items["add-kpi-" + i + "-before"].name = "Before";
					...
					menu.add.items["add-kpi-" + i].items["add-kpi-" + i + "-after"].name = "After";
			...
			$.each(hierarchies, function(i, hierarchy) {
				...
				if(levels.length == 1 && levels[0].name == hierarchy && levels[0].depth == 1) {
					...
					if($.inArray(hierarchy, axisHierarchies) > -1) {
						...
						menu.pivot.items["add-pivot-" + i].items["add-pivot-" + i + "-before"].name = "Before";
						...
						menu.pivot.items["add-pivot-" + i].items["add-pivot-" + i + "-after"].name = "After";
					...
					$.each(levels, function(j, level) {
						...
						if($.inArray(hierarchy, axisHierarchies) > -1) {
							...
							menu.pivot.items["add-pivot-" + i].items["add-pivot-" + i + "-" + j].items["add-pivot-" + i + "-" + j + "-before"].name = "Before";
							...
							menu.pivot.items["add-pivot-" + i].items["add-pivot-" + i + "-" + j].items["add-pivot-" + i + "-" + j + "-after"].name = "After";
        ...
        menu.toggleFilters = {
            name: (myself.properties.showFilters ? "Hide" : "Show") + " Filters and Sorts", 
            callback: function() {
		...
		menu.settings = {name: "Table Settings", items: settingsMenu.items};
		...
		return {
		    callback: function(key, options) {
		    	if(targetType == "DIMENSION" && !hasMoC && target.hasOwnProperty("index")) {
					...
					var url = webAppPath + "/content/BTable/render?";
	...
	myself.buildDrillMenu = function() {
 	...
 	myself.buildNoDataContextMenu = function(target) {	  	
		...
		menu.title = {name: "<strong>No data</strong>", callback: function(key, options) {}};
		menu.titlesep = "---";
